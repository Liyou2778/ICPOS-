...
**③b 检索缺陷修复（由评测链路发现，非人工猜测）**：问"E106 故障码对应什么故障"曾命中 E107/E111 ——
根因是关键词召回只用中文 2-gram，**E106 被切成 E1/11/10**，"故障"这类高频词主导打分；且弱哈希向量召回里
没有这些代码条目，融合后被相邻码反超（E106 仅 0.400，E107 靠 0.14 向量分 + 0.27 关键词分 = 0.408）。
修复三件套（`backend/app/services/rag.py`）：① `query_codes()` 抽取故障码/型号/工单号 token；
② 关键词打分改 **IDF 加权**（高频词降权）+ 精确 token 强加权；③ 融合阶段"**精确 token 优先**"（命中 +0.5）。
效果：同一批 60 条评测样本**数字覆盖率 0.632 → 0.895**；top_k 扫描（3/5/8）覆盖率同为 0.895，
说明剩余缺口属**知识覆盖**问题（如徐工官网解决方案页未入索引），不是"取少了"。
回归门禁：`backend/tests/test_retrieval_quality.py`（7 项，含覆盖率 ≥0.85 门槛）。
...
配置：4-bit QLoRA（bitsandbytes 可用）→ LoRA r=16/alpha=32，`max_len 512`、batch 1 × 梯度累积 8、
梯度检查点、**每 50 步保存检查点**（会话中断可续训——本机实测中断会连带杀掉后台训练，已发生 3 次）。
⚠️ transformers 5.x 已移除 `warmup_ratio`（改 `warmup_steps`），且 4-bit 下 `parameters()` 的 numel
是打包存储单元数、**不能当参数量对外报**（脚本改为按 config 估算 1.65B 并单独标注）。

**⑥ 微调结果与门控（诚实结论：不上线）**：train_loss 2.294 → **0.386**，耗时 **3405 s**，1.72 样本/秒，
adapter 70.5 MB。三方评测（同一批 60 条：检索上界 / 基座 / 微调后）：

| 指标 | 检索上界 | 基座 | 微调后 |
|---|---|---|---|
| 数字命中率 | 0.8947 | **0.8947** | 0.6316 ❌ |
| 字符 bigram F1 | 0.1687 | **0.2784** | 0.1933 |
| 拒答正确率 | 0.0000 | 0.5000 | **1.0000** ✅ |
| 正样本误拒率 | 0.0000 | 0.0556 | **0.3519** ❌ |

微调学会了拒答，但**过度拒答**：故障码问答（评测主力 48/60）的拒答率从 2.1% 升到 29.2%，命中率 1.000→0.750。
根因：训练集 85% 是项目编号查询（5000/5852），与评测集分布错配；拒答负样本 532 条 vs 故障案例正样本仅 210 条。
**处置：adapter 门控关闭、不上线，生产维持「基座 + 已修复检索」**；修复方案见报告 §6。
完整数据见 [`docs/validation/agent_sft_training_report.md`](validation/agent_sft_training_report.md)。
