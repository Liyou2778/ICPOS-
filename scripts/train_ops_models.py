...
def _cv_reg(X, y, make_model, folds: int = 5) -> dict:
    ...
    return {
        "cv_mae_mean": round(float(arr.mean()), 4),
...
def main() -> int:
    ...
    df = load_frame()
    print(f"[train_ops_models] 项目运营记录 {len(df)} 条；split={df['split'].value_counts().to_dict()}")
    ...
    print("\n=== EDA ===")
    print(
        f"  工期偏差(天)：均值 {y_delay.mean():.2f}，中位数 {y_delay.median():.1f}，"
        f"P90 {y_delay.quantile(0.9):.1f}，范围 [{y_delay.min():.0f}, {y_delay.max():.0f}]"
    )
    print(
        f"  延期比例 {y_delay_flag.mean():.3f}；超支比例 {y_overrun.mean():.3f}；"
    ...
    def gbr():
        return GradientBoostingRegressor(
            n_estimators=300, max_depth=3, learning_rate=0.05, subsample=0.9, random_state=20260918
        )
    ...
    print(
        f"B 成本偏差率回归：测试 MAE {res_b['mae']}（基线 {res_b['baseline_mae']}）R² {res_b['r2']}；"
    ...
    print(
        f"C 延期二分类：acc {res_c['accuracy']}（基线 {res_c['baseline_accuracy']}）AUC {res_c['auc']} "
    ...
    print(
        f"D 超支二分类：acc {res_d['accuracy']}（基线 {res_d['baseline_accuracy']}）AUC {res_d['auc']} "
    ...
    gate = {
        ...
        "cost_ratio_regression": bool(
            res_b["beats_baseline"] and res_b["cv_mae_ci95"][1] < res_b["baseline_mae"]
        ),
    ...
    joblib.dump(
        {
            ...
            "target": "schedule_deviation_days",
            "deployed": gate["delay_regression"],
        },
        OUT / "delay_reg.joblib",
    )
    joblib.dump(
        {
            "model": model_b,
            "feature_cols": feat_cols,
            "target": "cost_ratio",
            "deployed": gate["cost_ratio_regression"],
        },
        OUT / "cost_ratio_reg.joblib",
    )
    joblib.dump(
        {
            "model": clf_c,
            "feature_cols": feat_cols,
            "target": "delay_flag",
            "deployed": gate["delay_classification"],
        },
        OUT / "delay_clf.joblib",
    )
    joblib.dump(
        {
            "model": clf_d,
            "feature_cols": feat_cols,
            "target": "overrun_flag",
            "deployed": gate["overrun_classification"],
        },
        OUT / "overrun_clf.joblib",
    )
    ...
    base_model = {
        "schedule_deviation_days": {
            ...
            **{f"p{int(q * 100)}": round(float(y_delay[tr].quantile(q)), 2) for q in (0.5, 0.75, 0.9, 0.95)},
        ...
        "by_construction_task": {
            ...
            for task, g in df[tr].groupby("construction_task")
            if len(g) >= 20
        },
        "meaning": "运营基准：工期偏差与成本偏差分位数（训练集标定），用于排期缓冲与预算预警",
    }
    (OUT / "baseline_model.json").write_text(
        json.dumps(base_model, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    ...
    (OUT / "meta.json").write_text(
        json.dumps(
            {
                "feature_cols": feat_cols,
                "generated_at": report["trained_at"],
                "leakage_guard": report["dataset"]["leakage_guard"],
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    print(
        f"运营基准：工期偏差 P50 {base_model['schedule_deviation_days']['p50']} 天 / "
