...
DEMO_TEXT = ("我是矿山生产主管，新接到矿区开采任务：年产 200 万吨矿石，工期 3 年，"
...
def main() -> None:
    ...
    print(f"[dashboard] 设备 {d['device_total']} 台 / 开放预警 {d['warnings_open']} / "
    ...
    print(f"[solution] {len(payload['bundles'])} 套方案，耗时 {plan['latency_s']:.2f}s，"
    ...
    # 4) 投标方案响应度检查
    r = c.post("/api/solutions/plan", json={"text": "年产300万吨剥离，工期2年，预算2亿元，请生成投标方案",
                                            "doc_type": "bid"}, headers=H)
    bid = r.json()
    print(f"[bid] 章节 {len(bid['payload']['chapters'])}，响应度检查 {len(bid['payload']['bid_response_check'])} 条全部已响应")

    # 5) 文档导出（Word；LibreOffice 有则 PDF）
    r = c.post("/api/solutions/export", json={"payload": payload, "doc_type": "construction",
                                              "title": payload["title"]}, headers=H)
    ex = r.json()
    ...
    print(f"[dispatch] 派单 {len(r['assignments'])} 条，空载率 {r['stats']['idle_rate']*100:.1f}%，"
    ...
    print(f"[ab] 人工空载率 {ab['metrics']['manual_idle_rate']*100:.1f}% -> AI "
          f"{ab['metrics']['ai_idle_rate']*100:.1f}%，下降 {ab['improvements']['idle_rate_drop']*100:.1f}%")

    # 7) 运维：诊断/工单/预测
    diag = c.post("/api/maintenance/diagnose", json={"text": "液压油温高，动作没劲，怀疑漏油"}, headers=H).json()
    print(f"[diagnose] Top3: " + "；".join(f"{x['code']} {x['name']}({x['confidence']*100:.0f}%)" for x in diag["top3"]))
    wo = c.post("/api/maintenance/workorders", json={"device_code": "T02", "code": "HYD-01"}, headers=H).json()
    print(f"[workorder] {wo['code']} 设备{wo['device_code']} 工程师={wo['engineer']} 备件={len(wo['parts'])} 件")
    pr = c.get("/api/maintenance/predict/T04", headers=H).json()
    print(f"[predict] T04 risky={pr.get('risky')} code={pr.get('top_code')} conf={pr.get('top_conf')} "
    ...
    events: list[str] = []
    with c.stream("POST", f"/api/chat/sessions/{sid}/messages/stream",
                  json={"message": DEMO_TEXT}, headers=H) as resp:
