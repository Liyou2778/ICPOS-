...
def main() -> int:
    ...
    h = {
        ...
        + c.post("/api/auth/login", json={"username": "admin", "password": "icops2026"}).json()["token"]
    }
    ...
    plan = c.post(
        "/api/solutions/plan", json={"text": DEMO_TEXT, "doc_type": "construction"}, headers=h
    ).json()
    ok &= check(
        "方案生成 3 套 + 章节",
        plan["status"] == "ok"
        and len(plan["payload"]["bundles"]) >= 3
        and len(plan["payload"]["chapters"]) >= 5,
    )
    ex = c.post(
        "/api/solutions/export",
        json={"payload": plan["payload"], "doc_type": "construction", "title": "验收"},
        headers=h,
    ).json()
    ...
    ok &= check(
        "调度 A/B 空载率下降 ≥15%",
        ab["improvements"]["idle_rate_drop"] >= 0.15,
    ...
    wo = c.post(
        "/api/maintenance/workorders", json={"device_code": "T02", "code": "ENG-03"}, headers=h
    ).json()
